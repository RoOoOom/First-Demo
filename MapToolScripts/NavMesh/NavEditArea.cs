	...
	public class NavEditArea
	{
		...
		public bool CheckLineCross()
		{
			...
			for (int i = 0; i < iPointCount - 2; i++)
			{
				...
				p1s.y = this.m_lstPoints[i].transform.position.y;
				p1e.x = this.m_lstPoints[i + 1].transform.position.x;
				p1e.y = this.m_lstPoints[i + 1].transform.position.y;
				...
				for (int j = i + 2; j < iPointCount; j++)
				{
					if (j != iPointCount - 1)
					{
						...
						p2s.y = this.m_lstPoints[j].transform.position.y;
						p2e.x = this.m_lstPoints[j + 1].transform.position.x;
						p2e.y = this.m_lstPoints[j + 1].transform.position.y;
					}
					else
					{
						p2s.x = this.m_lstPoints[j].transform.position.x;
						...
						p2e.y = this.m_lstPoints[0].transform.position.y;
