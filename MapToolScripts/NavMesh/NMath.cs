...
namespace Game.NavMesh
{
    ...
    public class NMath
    {
        private const float EPSILON = 1e-005f;  //浮点数最小常量
        ...
        /// <returns></returns>
        public static Vector2 ExtendPos(Vector2 startPos, Vector2 tarPos, float length)
        {
        ...
        /// <summary>
        /// 记r = sin(夹角t，p1方向为正，顺时针旋转), 向量p1 = (a1, b1), 向量p2 = (a2, b2)
        /// 向量叉乘：向量p1 * 向量p2 = |p1| * |p2| * r
        /// 向量叉乘：向量p1 * 向量p2 = a1 * b2 - b1 * a2 , 记为Z
        /// Z > 0 -> r > 0: 点在向量p1右侧
        /// Z = 0 -> r = 0: 点在向量p1上
        /// Z < 0 -> r < 0: 点在向量p1左侧
