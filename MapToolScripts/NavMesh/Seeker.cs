...
namespace Game.NavMesh
{
    ...
    public class Seeker
    {
        private const float START_POS_EXTEND_LENGTH = 0f; //起始位置容错长度
        private const float END_POS_EXTEND_LENGTH = 0f;    //终点位置容错长度

        public List<NavTriangle> NavMeshData { set { this.m_lstTriangle = value; } }
        private List<NavTriangle> m_lstTriangle = new List<NavTriangle>();   //地图数据
        ...
        /// <returns>结果</returns>
        private PathResCode FixPos(ref NavTriangle orgTarTri, ref Vector2 orgTarPos, NavTriangle otherTri, Vector2 otherPos, float extendLength)
        {
            ...
            , List<NavTriangle> triPathList, out List<Vector2> wayPoints, int offSet)
        {
            ...
            for (int i = 0; i < triPathList.Count; i++)
            {
                ...
                if (i != triPathList.Count - 1)
                {
                    ...
                    Line2D outSide = tri.GetSide(tri.GetOutWallIndex());
                    dd += tri.GetID() + ", {(" + outSide.GetStartPoint().x / 100 + "," + outSide.GetStartPoint().y / 100 * 2 + "),(" + outSide.GetEndPoint().x / 100 + "," + outSide.GetEndPoint().y / 100 * 2 + ")}, ";
                }
               
            }
            Debug.Log("out dor " + dd);
        ...
        /// <returns></returns>
        public NavTriangle GetTriangleByPos(Vector3 pos)
        {
            if (null != m_lstTriangle && m_lstTriangle.Count > 0)
            {
                for (int i = 0; i < this.m_lstTriangle.Count; i++)
                {
                    if (m_lstTriangle[i].IsPointIn(pos))
                    {
                        return m_lstTriangle[i];
                    }
                }
